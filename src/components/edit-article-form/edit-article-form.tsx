...
export function EditArticleForm() {
  ...
  const { status, error, isRedirectNeeded } = useAppSelector((state) => state.article)
  ...
  const cbTags = (array: string[]) => array.map((item: string) => ({ tag: item }))
  ...
  return (
    <>
      {status === 'rejected' && <ErrorBlock error={error} />}
